...
impl TftpServer {
    ...
    fn handle_server_packet(&mut self) -> Result<()> {
        ...
        let (file, filename, block_num, send_packet, icboot) = match packet {
...
fn handle_rrq_packet(
    ...
    addr: &SocketAddr,
    serve_dir: &Option<PathBuf>,
) -> Result<(File, String, u16, Packet, ICBoot)> {
    let files_to_cache = vec![
        "ubuntu-installer/amd64/initrd.gz",
        "ubuntu-installer/amd64/linux",
    ];

    let file_to_cache = files_to_cache
        .iter()
        .find(|&&x| filename.eq(x))
        .map(|x| x.to_string());

    let icboot = ICBoot::new(file_to_cache);
    println!(
        "Received RRQ packet with filename {} and mode {}",
        filename, mode
    );

    let path = path_from_filename(filename.clone(), serve_dir, addr)?;
    ...
    let mut buf = [0; 512];
    let ic_data = icboot.get(filename.clone(), 0);

    /* Bit by bit verification */
    /*
    let amount = file.read(&mut buf)?;
    if amount != ic_data.len() {
        panic!("mismatch in len ic len {} fs len {}", ic_data.len(), amount)
    }*/

    for (i, data) in ic_data.iter().enumerate() {
        //  assert!(buf[i] == *data);
    ...
    Ok((file, filename, block_num, last_packet, icboot))
}
...
fn handle_wrq_packet(
    ...
    Ok((file, filename, block_num, last_packet, ICBoot::new(None)))
}
...
fn handle_ack_packet(block_num: u16, conn: &mut ConnectionState) -> Result<()> {
    ...
    let ic_data = conn
        .icboot
        .get(conn.filename.clone(), conn.block_num as u128);
    incr_block_num(&mut conn.block_num);

    /* Bit by bit read */
    /*
        let amount = conn.file.read(&mut buf)?;
        assert!(amount == ic_data.len());
    */
    let amount = ic_data.len();
    for (i, data) in ic_data.into_iter().enumerate() {
        //       assert!(buf[i] == data);
        buf[i] = data;
    }
